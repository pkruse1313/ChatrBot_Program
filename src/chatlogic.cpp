...
ChatLogic::~ChatLogic()
{
    ...
    // delete all nodes - not needed with smart pointers now
    /*for (auto it = std::begin(_nodes); it != std::end(_nodes); ++it)
    {
        delete *it;
    }*/

    // delete all edges - not needed with smart pointers now
...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
            ...
            if (type != tokens.end())
            {
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "NODE")
                    {
                        ...
                        //std::shared_ptr<MyType> const& p
                        auto newNode = std::find_if(_nodes.begin(), _nodes.end(), [&id](std::unique_ptr<GraphNode> const& p) { return p->GetID() == id; });
                    ...
                    if (type->second == "EDGE")
                    {
                        ...
                        if (parentToken != tokens.end() && childToken != tokens.end())
                        {
                            ...
                            auto parentNode = std::find_if(_nodes.begin(), _nodes.end(), [&parentToken](std::unique_ptr<GraphNode> const& p) { return p->GetID() == std::stoi(parentToken->second); });
                            auto childNode = std::find_if(_nodes.begin(), _nodes.end(), [&childToken](std::unique_ptr<GraphNode> const& p) { return p->GetID() == std::stoi(childToken->second); });
                            // create new edge
                            ...
                            _edges.push_back(std::shared_ptr<GraphEdge>(edge));
    ...
    this->SetChatbotHandle(_chatBot.get());
